...
class Lexicon {
	...
	function as_lexicon_setup() {
		...
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
			...
			$insert = "INSERT INTO " . $table_name .
			            " (keyword,content) " .
			            "VALUES ('".base64_encode("question")."','".base64_encode("the question is what is the question")."')";
	...
	function as_lexicon_manage() {
		...
		if ($_GET["action"] == "edit") {
			if ($_GET["updateData"] == "true") {
				...
				if (is_numeric($_GET["id"])==true) {
					...
					if ($status != "failed") {
						$id = $_GET["id"];
						$keyword = base64_encode($_POST["post_title"]);
		...
		while(list($id,$keyword)=mysql_fetch_array($res)) {
			array_push($tableData, array("id"=>$id,"keyword"=>base64_decode($keyword)));
		}
	...
	function as_lexicon_look_for_keywords($content) {
		...
		while(list($id,$keyword)=mysql_fetch_array($res)) {
			...
			$dKeyword = str_replace(" ", "-", $keyword);
			if ($seo==false) {
				$content = str_replace($keyword,"<a href=\"/?asenciclopedia=true&id=$id\">$keyword</a>", $content);
			} else {
				$content = str_replace($keyword,"<a href=\"/lexicon/$dKeyword-$id/\">$keyword</a>", $content);
			}
		}
		return $content;
	}
	...
	function as_lexicon_influence_posts($posts) {
		...
		if (is_numeric($wp_query->query_vars['id'])==true) {
			...
			$keyword = base64_decode($dbkeyword);
			$content = base64_decode($dbcontent);
			$showFooter = false;
